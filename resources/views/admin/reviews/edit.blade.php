...
@section('page-description', 'Update customer review - ' . $review->customer_name)
...
@section('content')
<!-- Edit Review Form -->
<form action="{{ route('admin.reviews.update', $review) }}" method="POST" class="space-y-8">
    @csrf
    @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="package_id" id="package_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200" required>
                    ...
                    <option value="{{ $package->id }}" {{ old('package_id', $review->package_id) == $package->id ? 'selected' : '' }}>
                        {{ $package->title }}
                    </option>
                    @endforeach
                </select>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                ...
                <input type="text" name="customer_name" id="customer_name" value="{{ old('customer_name', $review->customer_name) }}" 
            ...
            <div>
                ...
                <input type="email" name="customer_email" id="customer_email" value="{{ old('customer_email', $review->customer_email) }}" 
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200" 
            ...
            <div>
                ...
                <input type="text" name="customer_location" id="customer_location" value="{{ old('customer_location', $review->customer_location) }}" 
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200" 
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="mb-6">
            ...
            <textarea name="review_text" id="review_text" rows="6" 
                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200" 
                      placeholder="Write the customer's review here..." required>{{ old('review_text', $review->review_text) }}</textarea>
            @error('review_text')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_verified" id="is_verified" value="1" {{ old('is_verified', $review->is_verified) ? 'checked' : '' }}
            ...
            <!-- Featured Status -->
            <div class="flex items-center">
                <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $review->is_featured) ? 'checked' : '' }}
                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
            ...
            <!-- Approved Status -->
            <div class="flex items-center">
                <input type="checkbox" name="is_approved" id="is_approved" value="1" {{ old('is_approved', $review->is_approved) ? 'checked' : '' }}
                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
